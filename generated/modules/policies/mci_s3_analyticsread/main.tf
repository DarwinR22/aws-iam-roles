...
resource "aws_iam_policy" "main" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      {
Sid    = "S3AnalyticsRead"        Effect = "Allow"
        ...
        Condition = {
          StringEquals = {
            "aws:PrincipalTag/Gerencia" = var.abac_conditions["aws:PrincipalTag/Gerencia"]
,            "aws:PrincipalTag/Ambiente" = ["${environment}"]
          }
,          StringLike = {
            "s3:prefix" = ["${area}/*", "shared/*", "public/*"]
          }
        }
      }    ]
  })
